...
[[nodiscard]] bool is_defcon_lowering_card(CardId card_id) {
    return decode::is_defcon_lowering_card(card_id);
}

[[nodiscard]] bool is_card_blocked_by_defcon(const PublicState& pub, Side side, CardId card_id) {
    return decode::is_card_blocked_by_defcon(pub, side, card_id);
}
...
ActionEncoding build_action_from_country_logits(
    ...
    return decode::build_action_from_country_logits(
        card_id, mode, country_logits, pub, side, strategy_logits, country_strategy_logits);
}
