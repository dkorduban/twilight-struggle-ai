...
# --- Run 1: GNN 2x data 47ep (matched compute to 1x95) ---
log_start "RUN 1: GNN 2x47 s42"
nice -n 10 uv run python scripts/train_baseline.py \
    --data-dir data/nash_bc_combined \
    --out-dir data/checkpoints/v110_cf_gnn_2x47_s42 \
    --model-type control_feat_gnn --hidden-dim 256 \
    --batch-size 8192 --lr 0.0024 --epochs 47 --patience 15 \
...
benchmark_model "v110_gnn_2x47_s42" "data/checkpoints/v110_cf_gnn_2x47_s42"

# --- Run 2: GNN 3x data 32ep (matched compute to 1x95) ---
log_start "RUN 2: GNN 3x32 s42"
nice -n 10 uv run python scripts/train_baseline.py \
    --data-dir data/nash_bcd_combined \
    --out-dir data/checkpoints/v110_cf_gnn_3x32_s42 \
    --model-type control_feat_gnn --hidden-dim 256 \
    --batch-size 8192 --lr 0.0024 --epochs 32 --patience 15 \
    --dropout 0.1 --weight-decay 1e-4 --label-smoothing 0.05 \
    --one-cycle --deterministic-split --value-target final_vp --seed 42
log_done "Training Run 2"
benchmark_model "v110_gnn_3x32_s42" "data/checkpoints/v110_cf_gnn_3x32_s42"

# --- Run 3: GNN 2x data 95ep (longer training) ---
log_start "RUN 3: GNN 2x95 s42"
nice -n 10 uv run python scripts/train_baseline.py \
    --data-dir data/nash_bc_combined \
...
# --- Run 4: GNN 3x data 95ep (max data + max training) ---
log_start "RUN 4: GNN 3x95 s42"
nice -n 10 uv run python scripts/train_baseline.py \
    --data-dir data/nash_bcd_combined \
