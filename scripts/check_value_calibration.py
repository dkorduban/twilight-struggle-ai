...
from tsrl.policies.model import PlattScaler, TSBaselineModel
...
def apply_platt_scaling(preds: np.ndarray, a: float, b: float) -> np.ndarray:
    scaler = PlattScaler(a=a, b=b)
    with torch.no_grad():
        calibrated = scaler(torch.as_tensor(preds, dtype=torch.float32))
    return calibrated.cpu().numpy().astype(np.float32, copy=False)


def calibration_report(
    preds: np.ndarray, actuals: np.ndarray, label: str, n_bins: int = 10
) -> dict:
    ...
    total = len(preds)
    ece = sum(
        r["n"] / total * abs(r["pred_mean"] - r["actual_mean"]) for r in results if r["n"] > 0
    )
    ...
    for r in results:
        ...
        print(
            f"  [{lo:+.1f},{hi:+.1f})  {r['n']:>6}  {r['pred_mean']:>+.3f}  {r['actual_mean']:>+.3f}  {diff:>+.3f}  {''.join(chart)}"
        )

    return {"label": label, "n": total, "ece": ece, "brier": brier, "bias": bias, "bins": results}


def infer_generation_from_checkpoint(ckpt_path: str) -> str:
    return Path(ckpt_path).parts[-2].replace("retrain_", "")


def autodetect_checkpoint() -> str:
    hist = json.load(open("results/benchmark_history.json"))
    best_gen = max(hist.keys(), key=lambda k: hist[k]["learned_vs_heuristic"])
    ckpt_path = f"data/checkpoints/retrain_{best_gen}/baseline_best.pt"
    print(f"Auto-selected checkpoint: {ckpt_path} (win%={hist[best_gen]['learned_vs_heuristic']})")
    return ckpt_path


def autodetect_data_dir(generation: str) -> str:
    for candidate in [f"data/combined_v{generation}", f"data/combined_v{int(generation[1:]) + 1}"]:
        if os.path.isdir(candidate) and list(Path(candidate).glob("*.parquet")):
            print(f"Auto-selected data dir: {candidate}")
            return candidate
    raise FileNotFoundError(f"Could not auto-detect combined data dir for generation {generation}")


def default_output_path(generation: str, suffix: str = "value_calibration") -> str:
    return f"results/{suffix}_{generation}.json"


def load_validation_frame(data_dir: str, val_frac: float) -> pl.DataFrame:
    import glob as _glob

    paths = sorted(_glob.glob(os.path.join(data_dir, "*.parquet")))
    print(f"Found {len(paths)} parquet files — scanning game_ids (lightweight pass)...")

    all_gids_sorted: list[str] = []
    for path in paths:
        gids = pl.read_parquet(path, columns=["game_id"])["game_id"].unique().sort().to_list()
        all_gids_sorted.extend(gids)
    all_gids_sorted = sorted(set(all_gids_sorted))
    n_val = max(1, int(len(all_gids_sorted) * val_frac))
    ...
    for path in paths:
        ...
        frame = _norm(frame.select(canonical)).filter(pl.col("game_id").is_in(val_gids))
        if len(frame) == 0:
            continue
        tag = "selfplay" if "selfplay" in Path(path).name else "vs_heuristic"
        frame = frame.with_columns(pl.lit(tag).alias("_source"))
...
def value_targets(df: pl.DataFrame) -> np.ndarray:
    return (df["final_vp"].cast(pl.Float32) / 20.0).clip(-1.0, 1.0).to_numpy()
...
def append_reports(
    ...
    *,
    ...
    all_results.append(
        calibration_report(preds, actuals, f"{label_prefix}Overall ({generation})", n_bins=n_bins)
    )
...
def main():
    ...
    parser.add_argument(
        "--calibration",
        default=None,
        help="Optional JSON file with fitted Platt scaling params {a, b}.",
    )
    parser.add_argument("--n-bins", type=int, default=10, help="Reliability diagram bin count.")
    ...
    gen = infer_generation_from_checkpoint(args.checkpoint)

    if args.data_dir is None:
        args.data_dir = autodetect_data_dir(gen)

    if args.out is None:
        args.out = default_output_path(gen)
    ...
    if args.calibration is not None:
        ...
        calibrated_preds = apply_platt_scaling(
            preds, float(calibration["a"]), float(calibration["b"])
        )
        append_reports(
            all_results,
            calibrated_preds,
