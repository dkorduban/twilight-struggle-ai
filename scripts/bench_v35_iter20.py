#!/usr/bin/env python3
"""Formal bench of v35_iter20 vs heuristic, 500g/side, canonical seeds."""
...
CKPT = "results/ppo_v35_peerpool_smallstep/ppo_final_scripted.pt"
...
out = {
    ...
    "kill_gate": 0.66,
    "passes_kill_gate": combined >= 0.66,
}
print(f"v35_iter20 USSR={ussr:.3f}  US={us:.3f}  combined={combined:.3f}  ({t2-t0:.1f}s)")
print(f"vs v32 (0.650): {combined - 0.650:+.4f}")
print(f"kill gate 0.66: {'PASS' if combined >= 0.66 else 'FAIL (plateau confirmed)'}")

Path("results/analysis").mkdir(parents=True, exist_ok=True)
Path("results/analysis/v35_iter20_formal_bench.json").write_text(json.dumps(out, indent=2))
print("wrote results/analysis/v35_iter20_formal_bench.json")
