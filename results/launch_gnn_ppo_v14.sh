#!/bin/bash
# GNN PPO v14: control_feat_gnn_side architecture warm-started from BC
# Prerequisites: results/bc_gnn_side_v1/baseline_best.pt must exist
# Launch after BC training completes (~2026-04-20 08:08 UTC)

set -euo pipefail
cd "$(dirname "$0")/.."

BC_CHECKPOINT="results/bc_gnn_side_v1/baseline_best.pt"
if [ ! -f "$BC_CHECKPOINT" ]; then
    echo "ERROR: BC checkpoint not found: $BC_CHECKPOINT"
    exit 1
fi

mkdir -p results/ppo_gnn_side_v14

echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] Launching GNN PPO v14" | tee -a results/autonomous_decisions.log
...
OMP_NUM_THREADS=6 KMP_BLOCKTIME=0 nohup uv run python scripts/train_ppo.py \
  ...
  --reset-optimizer \
  --out-dir results/ppo_gnn_side_v14 \
  --version gnn_side_v14 \
  --n-iterations 80 \
  --games-per-iter 200 \
  --ppo-epochs 4 \
  --clip-eps 0.12 \
  ...
  --wandb-run-name ppo_gnn_side_v14_seed42000 \
  --league results/ppo_gnn_side_v14 \
  ...
  --max-kl 0.1 \
  --skip-smoke-test \
  > results/ppo_gnn_side_v14/train.log 2>&1 &

GNN_PID=$!
echo "GNN PPO v14 PID: $GNN_PID" | tee -a results/autonomous_decisions.log
echo "$GNN_PID" > results/ppo_gnn_side_v14/pid.txt
